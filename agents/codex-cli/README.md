...
docker run --rm -it -v $(pwd):/app -v $HOME/.codex:/home/node/.codex -e OPENAI_API_KEY=$OPENAI_API_KEY codex-cli -a never --sandbox danger-full-access

# If already logged in, run without API key
docker run --rm -it -v $(pwd):/app -v $HOME/.codex:/home/node/.codex codex-cli -a never --sandbox danger-full-access

# Or authenticate via device flow
docker run --rm -it -v $(pwd):/app -v $HOME/.codex:/home/node/.codex codex-cli login --device-auth --sandbox danger-full-access
```
