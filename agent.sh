...
build_agent() {
    ...
    read -rp "Select agent to build (1-8): " choice
    ...
    case "$choice" in
        ...
        4) docker build --no-cache -t devstral-cli:latest agents/devstral-cli ;;
        5) docker build --no-cache -t gemini-cli:latest agents/gemini-cli ;;
        6) docker build --no-cache -t kimi-cli:latest agents/kimi-cli ;;
        7) docker build --no-cache -t kiro-cli:latest agents/kiro-cli ;;
        8) docker build --no-cache -t qwen-code:latest agents/qwen-code ;;
